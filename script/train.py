...
from network import mlp, cnn_mlp
...
def train():
    ...
    env = envTest(training=True, recording=save_test_video_path, depth_res = (40, 40), reward=reward_config)
    state_shape = env.observation_space.shape or env.observation_space.n
    action_shape = env.action_space.shape or env.action_space.n
    max_action = env.action_space.high[0]

    train_envs = SubprocVectorEnv(
        [lambda: envTest(training=True, reward=reward_config, depth_res = (40, 40)) for _ in range(ppo_config["Training Envs"])],
        norm_obs=False)

    test_envs = SubprocVectorEnv(
        [lambda: envTest(training=True, reward=reward_config, depth_res = (40, 40)) for _ in range(ppo_config["Test Envs"]-1)]
        + [lambda: envTest(training=True, recording=save_test_video_path, reward=reward_config, depth_res = (40, 40))],
